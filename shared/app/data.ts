...
export const navigationByRole: Record<(typeof USER_ROLES)[keyof typeof USER_ROLES], NavigationItem[]> = {
  [USER_ROLES.superAdmin]: [{ label: 'Control Center', route: APP_ROUTES.superAdminOverview, description: 'All schools, billing, analytics, and platform operations' }],
...
const platformAdmin = { ...baseUser({ id: 'super-admin-wail', role: USER_ROLES.superAdmin, school: platformBrand, schoolCode: 'PLATFORM', email: 'owner@schoolos.test', password: 'Welcome123!', accessCode: 'PLATFORM-OWNER-9001', displayName: 'Wail Sebbar', recovery: { sms: '+212600000001', whatsapp: '+212600000001' }, biometricEnabled: true, knownDevices: [device('device-platform-1', 'MacBook Pro', 'Casablanca', '2026-03-11T00:40:00.000Z')], workspaceSummary: { title: 'Super admin control center', description: 'Track schools, revenue, subscriptions, feature flags, support, and onboarding from one platform dashboard.', metrics: [{ label: 'All schools', value: '12', detail: 'Active campuses on the platform' }, { label: 'Revenue', value: 'MAD 3.4M', detail: 'This month across all plans' }, { label: 'Open tickets', value: '14', detail: 'Tickets in progress or waiting for school response' }], priorities: ['Approve the next onboarding batch for new partner schools.', 'Review subscription renewals and billing exceptions.', 'Audit feature flag rollout before the next release window.'] } }) }
